...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("codascan_scraper.log"), logging.StreamHandler()],
)
...
async def run():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            if (
                "page" in pagination_text.lower()
                or "results" in pagination_text.lower()
            ):
        ...
        try:
            ...
            containers = await page.query_selector_all(
                "[class*='result'], [class*='item'], [class*='list'], [class*='grid']"
            )
        ...
        try:
            parties_button = await page.wait_for_selector(
                "button:has-text('Parties')", timeout=5000
            )
            if parties_button:
                logger.info(
                    "   🔘 Clicking 'Parties' button to load all party results..."
                )
                ...
                logger.info(
                    f"   📊 After clicking 'Parties': Found {len(initial_links)} links"
                )
                ...
                if len(initial_links) > 20:
                    ...
                    logger.warning(
                        "   ⚠️ Still only showing 20 results, trying alternative approach..."
                    )
                    ...
                    try:
                        ...
                        logger.info(
                            f"   📊 After second click: Found {len(new_links)} links"
                        )
                    ...
                    try:
                        logger.info(
                            "   🔍 Trying to navigate to a different URL pattern..."
                        )
                        ...
                        logger.info(
                            f"   📊 Test URL results: Found {len(test_links)} links"
                        )
            ...
            logger.warning(
                f"   ⚠️ 'Parties' button not found, proceeding with current results: {e}"
            )
        ...
        while scroll_attempts < max_scroll_attempts:
            try:
                ...
                logger.info(
                    f"Found {len(new_links)} links after scroll {scroll_attempts + 1}"
                )
                ...
                logger.warning(
                    f"Error during scroll attempt {scroll_attempts + 1}: {e}"
                )
                scroll_attempts += 1
        ...
        for pass_num in range(1, max_passes + 1):
            ...
            for i in range(0, len(current_links), batch_size):
                ...
                try:
                    batch_results = await asyncio.gather(
                        *[process_single_page(url, browser, pass_num) for url in batch],
                    ...
                    for j, result in enumerate(batch_results):
                        if isinstance(result, Exception):
                            ...
                            logger.error(
                                f"   ❌ {party_name}: Exception in batch processing - {result}"
                            )
                            batch_results[j] = {
                                "party": party_name,
                    ...
                    for result in batch_results:
                        if isinstance(result, dict):
                            ...
                            if result.get("balance") == "Not found" or "Error:" in str(
                                result.get("balance", "")
                            ):
            ...
            if current_links:
                logger.info(
                    f"🔄 Pass {pass_num} complete. {len(current_links)} parties failed, retrying in pass {pass_num + 1}..."
                )
                # Add longer delay between passes
                await asyncio.sleep(5)
            else:
                logger.info(
                    f"✅ Pass {pass_num} complete. All parties processed successfully!"
                )
        ...
        try:
            with open("codascan_balances.txt", "w") as f:
                ...
                for result in results:
                    ...
                    f.write(
                        f"{result['party']}\t{partyid}\t{result['balance']}\t{result['url']}\n"
                    )
...
async def process_single_page(url, browser, pass_num=1):
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            logger.info(
                f"   ⏱️ Pass {pass_num}, Attempt {attempt + 1}/{max_retries} - Waiting {current_wait_time/1000}s for content..."
            )
            ...
            try:
                ...
                try:
                    await detail_page.wait_for_selector(
                        "[class*='balance']", timeout=5000
                    )
                except Exception as e:
                    try:
                        await detail_page.wait_for_selector(
                            "[class*='amount']", timeout=5000
                        )
                    except Exception as e:
                        # If we can't find balance elements, try a longer wait on retry
                        if attempt < max_retries - 1:
                            logger.info(
                                f"   🔍 No balance elements found, will retry with longer wait..."
                            )
                        pass
                    ...
                    for elem in number_elements[:50]:  # Check first 50 elements
                        try:
                            ...
                            if text and re.search(
                                ...
                                logger.info(
                                    f"   🔢 Found number element: {text[:100]}..."
                                )
                        except Exception as e:
                            continue
                except Exception as e:
                ...
                try:
                    ...
                    for button in buttons[:5]:
                        try:
                            button_text = await button.text_content()
                            if (
                                "load" in button_text.lower()
                                or "refresh" in button_text.lower()
                            ):
            ...
            if not text_content or len(text_content.strip()) < 100:
                if attempt < max_retries - 1:
                    logger.warning(
                        f"   ⚠️ {party_name}: Page content too short ({len(text_content)} chars), retrying..."
                    )
                    continue
                else:
                    logger.error(
                        f"   ❌ {party_name}: Page content too short after all retries"
                    )
                    return {"party": party_name, "balance": "Not found", "url": url}

            # Debug: Log what we found for problematic parties
            if party_name in ["flowdesk", "gemini", "hashnote", "noders", "redstone"]:
                logger.info(
                    f"   🔍 Debug - {party_name} text content (first 200 chars): {text_content[:200]}"
                )
                import re

                decimal_matches = re.findall(
                    r"([0-9]{1,3}(?:,[0-9]{3})*\.[0-9]+)", text_content
                )
                logger.info(
                    f"   🔢 Debug - {party_name} decimal numbers found: {decimal_matches[:5]}"
                )
            ...
            if balance:
                ...
                return {"party": party_name, "balance": balance, "url": url}
            else:
            ...
            error_msg = str(e)
            logger.warning(
                f"   ⚠️ {party_name}: Pass {pass_num}, Attempt {attempt + 1}/{max_retries} failed - {error_msg}"
            )

            # Check if it's a network error that might be retryable
            if any(
                network_error in error_msg.lower()
                for network_error in ["timeout", "network", "connection", "socket"]
            ):
                if attempt < max_retries - 1:
                    next_wait_time = (
                        wait_times[min(attempt + 1, len(wait_times) - 1)] / 1000
                    )
                    logger.info(
                        f"   🔄 Retrying {party_name} in 2 seconds with longer wait time ({next_wait_time}s)..."
                    )
    ...
    # All retries failed
    logger.error(
        f"   ❌ {party_name}: All {max_retries} attempts failed in pass {pass_num}"
    )
    return {"party": party_name, "balance": f"Error: {error_msg}", "url": url}
...
def extract_balance_from_html(html_content, text_content):
    ...
    if "bitsafe" in text_content.lower() or "minter" in text_content.lower():
        logger.debug(
            f"   🔍 Debug - Text content for bitsafe/minter: {text_content[:500]}..."
        )
            ...
            if matches:
                ...
                for match in matches:
                    try:
                        ...
                        if (
                            balance_float >= 0
                            and balance_float < 10000000
                            and not (balance_float >= 1900 and balance_float <= 2030)
                        ):  # Exclude years
                            return match
                    except ValueError:
                        continue
