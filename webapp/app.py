...
def get_telegram_code():
    ...
    try:
        ...
        if code:
            ...
            db.execute_query(
                cursor, "UPDATE telegram_audit_status SET code = NULL WHERE id = 1"
            )
            conn.commit()
        return code
    except Exception as e:
...
def get_telegram_password():
    ...
    try:
        ...
        cursor = db.get_cursor(conn)
        db.execute_query(
            cursor, "SELECT password FROM telegram_audit_status WHERE id = 1"
        )
        row = cursor.fetchone()
        row_dict = dict(row) if row and not isinstance(row, dict) else row
        ...
        if password:
            ...
            db.execute_query(
                cursor, "UPDATE telegram_audit_status SET password = NULL WHERE id = 1"
            )
            conn.commit()
        return password
    except Exception as e:
...
def get_telegram_session():
    ...
    try:
        ...
        cursor = db.get_cursor(conn)
        db.execute_query(
            cursor, "SELECT session_string FROM telegram_audit_status WHERE id = 1"
        )
        row = cursor.fetchone()
...
def dashboard():
    ...
    # Get employee stats
    db.execute_query(
        cursor, "SELECT COUNT(*) as count FROM employees WHERE status = 'active'"
    )
    row = cursor.fetchone()
    active_count = row["count"] if isinstance(row, dict) else row[0]

    db.execute_query(
        cursor, "SELECT COUNT(*) as count FROM employees WHERE status = 'inactive'"
    )
    row = cursor.fetchone()
    inactive_count = row["count"] if isinstance(row, dict) else row[0]

    db.execute_query(
        cursor, "SELECT COUNT(*) as count FROM employees WHERE status = 'optional'"
    )
    row = cursor.fetchone()
...
def employees():
    ...
    return render_template(
        "employees.html", employees=employees, status_filter=status_filter
    )
...
def audits():
    ...
    db.execute_query(
        cursor, "SELECT * FROM audit_runs ORDER BY started_at DESC LIMIT 1"
    )
    latest_audit = cursor.fetchone()
    if latest_audit:
        db.execute_query(
...
def audit_detail(audit_id):
    ...
    return render_template(
        "audit_detail.html", audit=audit, findings=findings, report_data=report_data
    )
...
def offboarding():
    ...
    db.execute_query(
        cursor, "SELECT * FROM employees WHERE status = 'inactive' ORDER BY name"
    )
    ...
    return render_template(
        "offboarding.html", inactive_employees=inactive_employees, tasks=tasks
    )
...
def api_start_telegram_audit():
    ...
    set_telegram_status(
        "requesting_code", "Requesting authentication code from Telegram..."
    )
                ...
                if status["status"] == "authenticating":
                    ...
                    if code:
                        try:
                            ...
                            set_telegram_status(
                                "running", "Authenticated! Running audit..."
                            )
                            break
                        except SessionPasswordNeededError:
                            # 2FA password is required
                            set_telegram_status(
                                "waiting_for_password", "Enter your 2FA password"
                            )
                            break
                        except Exception as e:
