...
def demo_company_search(data: Dict[str, Any]) -> None:
    ...
    search_term = "bit"
    matches = [name for name in data.get('companies', {}).keys() 
    ...
    for company in matches[:5]:  # Show first 5
        ...
        print(f"  • {company}: Slack({slack_count}) Telegram({telegram_count}) Calendar({calendar_count}) HubSpot({hubspot_count})")
...
def demo_data_analysis(data: Dict[str, Any]) -> None:
    ...
    for company_name, company_data in data.get('companies', {}).items():
        ...
        if slack_channels:
            total_messages = sum(channel.get('message_count', 0) for channel in slack_channels)
...
def demo_stage_analysis(data: Dict[str, Any]) -> None:
    ...
    for company_name, company_data in data.get('companies', {}).items():
        slack_channels = company_data.get('slack_channels', [])
        total_stages = sum(channel.get('stage_detection_count', 0) for channel in slack_channels)
        if total_stages > 0:
...
def demo_commission_analysis(data: Dict[str, Any]) -> None:
    ...
    for company in commission_ready_companies[:5]:
        print(f"  • {company['company_name']}: {company['stage_count']} stages, {company['message_count']} messages, {company['channels']} channels")
