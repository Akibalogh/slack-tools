...
SLACK_TOKEN = os.getenv(
    "SLACK_USER_TOKEN"
)  # Use user token with groups:read/history scopes
...
def categorize_group(group_name):
    ...
    if any(
        mg.lower() in group_name.lower()
        for mg in GROUP_CATEGORIES.get("marketing_groups", [])
    ):
        return "Marketing", False

    # Check internal groups
    if any(
        ig.lower() in group_name.lower()
        for ig in GROUP_CATEGORIES.get("internal_groups", [])
    ):
        return "Internal", False

    # Check intro groups
    if any(
        intro.lower() in group_name.lower()
...
class CustomerGroupAuditor:
    ...
    async def get_slack_bd_members(self):
                ...
                async with session.get(
                    ...
                    for user in data.get("members", []):
                        ...
                        if actual_username in SLACK_USERNAME_MAP:
                            ...
                            if (
                                mapped_handle in REQUIRED_SLACK_MEMBERS
                                and mapped_handle not in self.required_slack_ids
                            ):
                            ...
                            # Is it an optional member?
                                ...
                                mapped_handle in OPTIONAL_MEMBERS
                                and mapped_handle not in self.optional_slack_ids
                            ):
                                self.optional_slack_ids[mapped_handle] = user_id
                                print(
                                    f"   ✓ Found optional: {OPTIONAL_MEMBERS[mapped_handle]} (@{user.get('name')})"
                                )
        ...
        print(
            f"   Mapped {len(self.required_slack_ids)}/{len(REQUIRED_SLACK_MEMBERS)} required members"
        )
        print(
            f"   Mapped {len(self.optional_slack_ids)}/{len(OPTIONAL_MEMBERS)} optional members"
        )

        # Show any missing mappings
        missing_required = set(REQUIRED_SLACK_MEMBERS.keys()) - set(
            self.required_slack_ids.keys()
        )
        missing_optional = set(OPTIONAL_MEMBERS.keys()) - set(
            self.optional_slack_ids.keys()
        )
        ...
        async with aiohttp.ClientSession() as session:
            for channel in bitsafe_channels:
                ...
                async with session.get(
                    ...
                    if not data.get("ok"):
                        print(
                            f"   ⚠️  Couldn't access {channel_name}: {data.get('error')}"
                        )
                        continue
                ...
                self.audit_results.append(
                    {
                        ...
                        "Required Present": (
                            ", ".join(required_present) if required_present else "NONE"
                        ),
                        "Required Missing": (
                            ", ".join(required_missing) if required_missing else "-"
                        ),
                        "Optional Present": (
                            ", ".join(optional_present) if optional_present else "-"
                        ),
                        "Optional Missing": (
                            ", ".join(optional_missing) if optional_missing else "-"
                        ),
    ...
    async def audit_telegram_groups(self):
        ...
        try:
            ...
            while True:
                result = await client(
                    GetCommonChatsRequest(user_id=mojo_user, max_id=max_id, limit=100)
                )
            ...
            for idx, chat in enumerate(common_chat_ids, 1):
                ...
                try:
                    ...
                    print(
                        f"      Warning: Couldn't check history settings for {group_name}: {e}"
                    )
                    history_visible = "Unknown"
                ...
                try:
                    ...
                    admin_status = (
                        f"Unknown ({str(e)[:30]}...)" if str(e) else "Unknown"
                    )
                    ...
                    for p in participants:
                        ...
                        if username in REQUIRED_TELEGRAM_MEMBERS:
                            ...
                            if REQUIRED_TELEGRAM_MEMBERS[username] in required_missing:
                                required_missing.remove(
                                    REQUIRED_TELEGRAM_MEMBERS[username]
                                )
    ...
    def generate_report(self):
        ...
        public_count = len(
            df[
                (df["Platform"] == "Slack")
                & (df["Privacy Status"].str.contains("PUBLIC"))
            ]
        )
        if public_count > 0:
        ...
        hidden_history_count = len(
            df[
                (df["Platform"] == "Telegram")
                & (df["History Visibility"].str.contains("HIDDEN"))
            ]
        )
        if hidden_history_count > 0:
            print(
                f"   Telegram groups with HIDDEN history (should be visible): {hidden_history_count}"
            )
        ...
        if len(bd_groups) > 0:
            ...
            print(
                f"   BD Customer groups with missing members: {incomplete}/{len(bd_groups)}"
            )
