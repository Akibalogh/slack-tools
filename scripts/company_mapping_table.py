...
class CompanyMappingTable:
    ...
    def load_slack_channels(self) -> Dict[str, str]:
        ...
        for channel_name, msg_count in channels:
            if channel_name and msg_count > 0:
                ...
                company_name = channel_name.replace("-bitsafe", "").replace("-cbtc", "").replace("-ibtc", "")
                # Convert to title case for display
                display_name = " ".join(word.capitalize() for word in company_name.split("-"))
                ...
                try:
                    ...
                    for line in lines:
                        ...
                        if ' → ' in line and 'chat_' in line:
                            parts = line.split(' → ')
                            if len(parts) == 2:
                                company = parts[0].strip().replace('**', '').replace('`', '')
                                chat_dir = parts[1].strip().replace('`', '')
                                
                                # Extract company name directly from the line
                                extracted_company = self._extract_company_from_telegram_name(company)
                                if extracted_company and self._is_company_name(extracted_company):
                                    telegram_mapping[extracted_company] = chat_dir
                                    
                        # Pattern: Company | Group Name | Match Type
                        elif ' | ' in line and 'Match Type' not in line and 'Company' not in line:
                            parts = line.split(' | ')
                            if len(parts) >= 2:
                                company = parts[0].strip()
                                group_name = parts[1].strip()
                                if group_name and group_name != 'Chat Dir' and not group_name.startswith('chat_'):
                                    # Extract company name from the group name using the new method
                                    extracted_company = self._extract_company_from_telegram_name(group_name)
                                    if extracted_company and self._is_company_name(extracted_company):
    ...
    def _extract_company_from_telegram_name(self, telegram_name: str) -> str:
        ...
        clean_name = telegram_name.replace('&lt;', '<').replace('&gt;', '>').replace('&amp;', '&')
        ...
        for pattern in patterns:
            ...
            if match:
                ...
                company_name = re.sub(r'^(Khaled Verjee|Tally|Nico|Chris|John|Jonathan|Pinar|Karim|Mákkidamia|Levente|Simran|Joris|Klint|AlexC@|Nenter @|Alessio|Mike|Daniel|Faizan|Dom|Mateusz|Joo|Mihir|Gattman|PumpkinSeed|Ari|Yiannis|Jesse|Dim|Rex|Emmett|Anna|Dae|Luca|Trey|Jasminer|Forbole|Uwi|Nenter|IntellectEU|DeFiZoo\.fi|Pink Finance|Coinsummer|Chainup|F2pool|iPollo/Bitrise|Waterdrip|GBI|AngelHack|Superset|OKX DeFi|Solana DeFi|Ethereum Hungary|Harvard Blockchain|Magyar Internet 3|TRGC Familia|🇨🇭STS)\s+', '', company_name, flags=re.IGNORECASE)
                company_name = company_name.strip()
    ...
    def _is_company_name(self, name: str) -> bool:
        ...
        dm_indicators = [
            'Proof', 'DM', 'Direct', 'Message', 'Chat', 'Conversation'
        ]
        ...
        company_indicators = [
            'Labs', 'Capital', 'Group', 'Foundation', 'Protocol', 'DAO', 'DeFi', 'Finance',
            'Ventures', 'Partners', 'Systems', 'Technologies', 'Solutions', 'Network',
            'Exchange', 'Trading', 'Investment', 'Asset', 'Blockchain', 'Crypto',
            'BitSafe', 'iBTC', 'cBTC', 'dlcBTC', 'BitSafe', 'Canton',
            # Add specific companies we know exist
            'Allnodes', 'Republic', 'Nethermind', '7Ridge', 'Chainsafe', 'Redstone',
            'Sendit', 'Mpch', 'Register Labs', 'Chata Ai', 'Ordinals Foundation',
            'Blockdaemon', 'Blockandbones', 'Blackmanta', 'Blacksand', 'Brale',
            'Cense', 'Chainexperts', 'Copper', 'Cygnet', 'Digik', 'Entropydigital',
            'Five North', 'G20', 'Gateway', 'Hashrupt', 'Hlt', 'Integraate',
            'Ipblock', 'Kaiko', 'Kaleido', 'Komonode', 'Launchnodes', 'Levl',
            'Lightshift', 'Lithiumdigital', 'Luganodes', 'Maestro', 'Matrixedlink',
            'Mintify', 'Mlabs', 'Modulo Finance', 'Mse', 'Neogenesis', 'Nodemonster',
            'Noders', 'Notabene', 'Novaprime', 'Obsidian', 'Openblock', 'P2p',
            'Palladium', 'Pathrock', 'Proof', 'Rwaxyz', 'Sbc', 'T Rize',
            'Tall Oak Midstream', 'Temple', 'Tenkai', 'Thetanuts', 'Trakx',
            'Ubyx', 'Vigilmarkets', 'Xlabs', 'Zeconomy'
        ]
        ...
        if len(clean_name) > 6 and (' ' in clean_name or '-' in clean_name or '_' in clean_name):
            return True
    ...
    def load_wallet_data(self) -> Dict[str, Dict]:
        ...
        try:
            ...
            for company_name, status, fund_ownership, billing_wallet, external_id, deposit, paid in wallets:
    ...
    def _find_company_matches(self, slack_companies: Dict[str, str], telegram_companies: Dict[str, str]) -> Dict[str, tuple]:
        ...
        for normalized_name in slack_normalized:
            if normalized_name in telegram_normalized:
                ...
                matches[normalized_name] = (slack_display, slack_channel, telegram_company, telegram_group)
        ...
        for slack_norm, (slack_display, slack_channel) in slack_normalized.items():
            if slack_norm not in matches:  # Skip if already matched
                for telegram_norm, (telegram_company, telegram_group) in telegram_normalized.items():
                    if telegram_norm not in [m[2] for m in matches.values()]:  # Skip if already matched
                        # Check if one name contains the other (partial match)
                        if (slack_norm in telegram_norm or telegram_norm in slack_norm) and len(slack_norm) > 3 and len(telegram_norm) > 3:
                            matches[f"{slack_norm}_partial"] = (slack_display, slack_channel, telegram_company, telegram_group)
    ...
    def generate_mapping_table(self) -> str:
        ...
        if company_matches:
            ...
            table_lines.append("| Company | Slack Channel | Telegram Group | Calendar Entries |")
            table_lines.append("|---------|---------------|----------------|------------------|")
            
            for normalized_name, (slack_display, slack_channel, telegram_company, telegram_group) in sorted(company_matches.items()):
                calendar_info = calendar_entries.get(slack_display, [])
                
                # Format calendar entries
                if calendar_info:
                    calendar_display = "<br>".join(calendar_info[:2])  # Show first 2 meetings
                    if len(calendar_info) > 2:
                        calendar_display += f"<br>... and {len(calendar_info) - 2} more"
                else:
                    calendar_display = ""
                
                table_lines.append(f"| **{slack_display}** (Overlap) | {slack_channel} | {telegram_group} | {calendar_display} |")
        ...
        if wallet_companies:
            ...
            table_lines.append("| Company | Slack Channel | Telegram Group | Calendar Entries | Wallet Status |")
            table_lines.append("|---------|---------------|----------------|------------------|---------------|")
            ...
            for company in sorted(wallet_companies, key=str.lower):
                ...
                if company == "Redstone":
                    ...
                    telegram_name = "POPS Telegram Group"  # User confirmed POPS has Telegram
                elif company == "Path Rock":
                    slack_name = "pathrock-bitsafe"  # User confirmed Path Rock has Slack
                elif company == "OpenVector (Cypherock)":
                    slack_name = "openvector-bitsafe"  # User confirmed OpenVector has Slack
                ...
                if wallet_info:
                    ...
                    wallet_display = f"{status} ({fund_ownership}) - {deposit} CC / {paid} CC"
                ...
                # Highlight overlaps
                is_overlap = any(slack_display == match[1] for match in company_matches.values())
                if is_overlap:
                    company_display = f"**{company}** (Overlap)"
                else:
                    company_display = company
                
                table_lines.append(f"| {company_display} | {slack_display} | {telegram_display} | {calendar_display} | {wallet_display} |")
            
            table_lines.append("")
            
            # Show coverage summary for wallet companies (excluding BitSafe)
            wallet_companies_filtered = [c for c in wallet_companies if c.lower() not in ['bitsafe', 'bitsafe-minter']]
            wallet_with_slack = sum(1 for company in wallet_companies_filtered if slack_channels.get(company, "") or company in ["Redstone", "Path Rock", "OpenVector (Cypherock)"])
            wallet_with_telegram = sum(1 for company in wallet_companies_filtered if telegram_companies.get(company, "") or company in ["POPS", "SenseiNode"])
            wallet_with_calendar = sum(1 for company in wallet_companies_filtered if calendar_entries.get(company, []))
            wallet_with_both = sum(1 for company in wallet_companies_filtered if (slack_channels.get(company, "") or company in ["Redstone", "Path Rock", "OpenVector (Cypherock)"]) and (telegram_companies.get(company, "") or company in ["POPS", "SenseiNode"]) and calendar_entries.get(company, []))
            
            table_lines.append("### Wallet Companies Coverage Summary (Excluding BitSafe)")
            table_lines.append(f"- **Total Wallet Companies**: {len(wallet_companies_filtered)}")
            ...
            table_lines.append(f"- **Missing Slack**: {len(wallet_companies_filtered) - wallet_with_slack}")
            table_lines.append(f"- **Missing Telegram**: {len(wallet_companies_filtered) - wallet_with_telegram}")
            table_lines.append("")
        ...
        table_lines.append("- **Slack**: Channel names ending with '-bitsafe', '-cbtc', or '-ibtc'")
        table_lines.append("- **Telegram**: Group names from CRM mapping and export analysis")
        table_lines.append("- **Calendar**: Meeting data from Google Calendar integration")
        table_lines.append("- **Wallet**: Billing wallet addresses and financial status from company agreements")
        table_lines.append(f"- **Overlaps**: {len(company_matches)} companies have both Slack and Telegram data")
        
        return "\n".join(table_lines)
    
    def save_mapping_table(self, table_content: str, output_file: str = "company_mapping_table.md"):
    ...
    def generate_csv_mapping_table(self) -> str:
        ...
        csv_content = []
        csv_content.append("Company Name,Slack Channel,Telegram Group,Calendar Entries,Wallet Status,Platform Overlap")
        ...
        for normalized_name, (slack_display, slack_channel, telegram_company, telegram_group) in sorted(company_matches.items()):
            ...
            csv_content.append(f'"{slack_display}","{slack_channel}","{telegram_group}","{calendar_text}","{wallet_status}","Slack + Telegram"')
        ...
        for company in sorted_companies:
            ...
            csv_content.append(f'"{company}","{slack_name}","{telegram_name}","{calendar_text}","{wallet_status}","{platform_overlap}"')
        ...
        for line in lines:
            if line.startswith('|'):
                ...
                if len(cells) == 5:
                    ...
                    print(f"{company:<25} | {slack:<20} | {telegram:<25} | {calendar:<30} | {wallet}")
                elif len(cells) == 4:
