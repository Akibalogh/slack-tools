...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("stage_detection.log"), logging.StreamHandler()],
)
...
class StageDetectionPopulator:
    ...
    def populate_stage_detections(self):
        ...
        cursor.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='stage_detections'"
        )
        if not cursor.fetchone():
            logger.error(
                "stage_detections table not found. Please run slack_ingest.py first."
            )
            return
        ...
        if message_count == 0:
            logger.error(
                "No messages found in database. Please run slack_ingest.py first."
            )
            return
        ...
        logger.info(
            f"Inserted {stage_detections_inserted} stage detections from {len(messages)} messages"
        )
    ...
    def show_stage_summary(self):
        ...
        if results:
            ...
            for stage_name, count, avg_confidence in results:
                logger.info(
                    f"{stage_name:20} | {count:4} detections | {avg_confidence:.2f} avg confidence"
                )
        else:
...
def main():
    ...
    if not Path(db_path).exists():
        print(
            f"❌ Database not found at {db_path}. Please provide correct path or run slack_ingest.py first."
        )
        return

    # Check if config exists (look in data/slack if using that path)
    config_path = "config.json"
    if "data/slack" in db_path:
        config_path = "data/slack/config.json"

    if not Path(config_path).exists():
        print(
            f"❌ Config file not found at {config_path}. Please create config.json first."
        )
        return

    # Initialize and run
    populator = StageDetectionPopulator(db_path, config_path)
    populator.populate_stage_detections()

    print("\n✅ Stage detection population complete!")
    print(
        "Now you can run repsplit.py to generate proper justifications with stage analysis."
    )
