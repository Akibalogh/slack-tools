...
def list_audits(db, status=None, limit=10):
    ...
    for audit in audits:
        ...
        print(
            f"{audit_dict['id']:<5} {audit_dict['run_type']:<10} "
...
def cancel_audit(db, audit_id):
    ...
    try:
        ...
        db.execute_query(
            cursor, "SELECT id, status FROM audit_runs WHERE id = ?", (audit_id,)
        )
        ...
        if audit_dict["status"] != "running":
            print(
                f"⚠️  Audit #{audit_id} is not running "
...
def cancel_all_running(db):
    ...
    try:
        ...
        db.execute_query(cursor, "SELECT id FROM audit_runs WHERE status = 'running'")
...
def main():
    parser = argparse.ArgumentParser(description="Manage customer group audits")
    ...
    list_parser.add_argument(
        "--limit", type=int, default=10, help="Number of audits to show (default: 10)"
    )

    # Cancel command
    cancel_parser = subparsers.add_parser("cancel", help="Cancel a running audit")
    cancel_parser.add_argument("audit_id", type=int, help="Audit ID to cancel")

    # Cancel all command
    subparsers.add_parser("cancel-all", help="Cancel all running audits")

    # Cleanup command
    subparsers.add_parser(
        "cleanup", help="Clean up Telegram session locks and reset status"
    )
