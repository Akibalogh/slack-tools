...
def create_sales_reps_table(db_path: str):
    ...
    try:
        cursor.execute("ALTER TABLE users ADD COLUMN is_sales_rep BOOLEAN DEFAULT FALSE")
    ...
    try:
        cursor.execute("ALTER TABLE stage_detections ADD COLUMN author_is_sales_rep BOOLEAN DEFAULT FALSE")
    except sqlite3.OperationalError:
        # Column already exists
        pass
    ...
    try:
        cursor.execute("ALTER TABLE messages ADD COLUMN author_is_sales_rep BOOLEAN DEFAULT FALSE")
    except sqlite3.OperationalError:
        # Column already exists
        pass
...
def verify_sales_reps(db_path: str):
    ...
    cursor.execute("SELECT user_id, display_name, real_name, role, is_active FROM sales_reps")
    ...
    cursor.execute("SELECT COUNT(*) FROM stage_detections WHERE author_is_sales_rep = TRUE")
    authorized_stages = cursor.fetchone()[0]
    
    cursor.execute("SELECT COUNT(*) FROM stage_detections WHERE author_is_sales_rep = FALSE")
