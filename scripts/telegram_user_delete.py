...
try:
    ...
    from telethon.errors import (
        ...
        UserAdminInvalidError, FloodWaitError
    )
except ImportError:
...
class TelegramOffboarding:
        ...
        if not await self.client.is_user_authorized():
            ...
            try:
                ...
                if 'SessionPasswordNeededError' in str(type(e).__name__):
                    logger.info("🔐 Two-factor authentication detected. Password required...")
                    password = os.getenv('TELEGRAM_PASSWORD')
                    if not password:
                        password = input('Enter your 2FA password: ')
                    else:
                        logger.info(f"🔐 Using password from TELEGRAM_PASSWORD environment variable")
    ...
    async def find_user(self, username: str) -> Optional[User]:
        ...
        try:
            ...
            if isinstance(user, User):
                logger.info(f"✅ Found user: {user.first_name} {user.last_name or ''} (@{user.username})")
    ...
    async def get_chat_admins(self, chat) -> List[str]:
        ...
        try:
            if isinstance(chat, Channel):
                ...
                async for participant in self.client.iter_participants(chat, filter=ChannelParticipantsAdmins):
            ...
            logger.debug(f"Could not get admins for {getattr(chat, 'title', 'Unknown')}: {e}")
        ...
        try:
            ...
            if isinstance(chat, Chat):
                ...
                try:
                    ...
                    logger.debug(f"   Cannot check permissions in {chat.title}: {type(perm_error).__name__}")
                    return False
            ...
            logger.debug(f"   Could not check permissions for {getattr(chat, 'title', 'Unknown')}: {type(e).__name__}")
            return False
    ...
    async def remove_user_from_chat(self, chat, user: User) -> bool:
        ...
        try:
            ...
            if self.dry_run:
                logger.info(f"🔍 [DRY RUN] Would remove @{user.username} from: {chat_name}")
                return True
    ...
    def print_summary(self):
        ...
        if self.no_admin_access_groups:
            logger.info("\n" + "=" * 70)
            logger.info(f"⚠️  GROUPS WITHOUT ADMIN ACCESS ({len(self.no_admin_access_groups)})")
            logger.info("=" * 70)
...
Examples:
    ...
    if not args.username:
        parser.error("Username is required. Use --username=USERNAME or set TARGET_USERNAME env var")
