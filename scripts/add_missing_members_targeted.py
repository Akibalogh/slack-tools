        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                if data.get("ok"):
                    ...
                    return {"status": "success", "channel": channel_name, "member": username}
                elif data.get("error") == "already_in_channel":
                    print(f"   ℹ️  {username} already in {channel_name}")
                    return {"status": "already_member", "channel": channel_name, "member": username}
                else:
                    error = data.get("error", "unknown")
                    print(f"   ❌ Failed to add {username} to {channel_name}: {error}")
                    return {"status": "error", "channel": channel_name, "member": username, "error": error}
    ...
    async def process_missing_members(self, report_file):
        ...
        for _, row in df.iterrows():
            ...
            for member_name in missing_members:
                ...
                result = await self.add_member_to_channel(channel_name, channel_id, username, user_id)
