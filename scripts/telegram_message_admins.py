...
try:
    ...
    from telethon.errors import FloodWaitError, UserIsBlockedError, UserPrivacyRestrictedError
except ImportError:
...
class TelegramAdminMessenger:
            ...
            try:
                ...
                if 'SessionPasswordNeededError' in str(type(e).__name__):
                    ...
                    if not password:
                        ...
                        logger.info("🔐 Using password from TELEGRAM_PASSWORD environment variable")
    ...
    async def send_message_to_admin(self, username: str, group_names: List[str]) -> bool:
        """
        ...
        try:
            ...
            if self.dry_run:
                groups_desc = f"{len(group_names)} group{'s' if len(group_names) > 1 else ''}"
                logger.info(f"🔍 [DRY RUN] Would message @{username} about: {groups_desc}")
    ...
    async def message_all_admins(self):
        ...
        sorted_admins = sorted(all_admins.items(), key=lambda x: len(x[1]), reverse=True)
        
        for username, groups in sorted_admins:
            group_count = len(groups)
            logger.info(f"@{username:20} → {group_count} group{'s' if group_count > 1 else ''}")
            if group_count <= 3:
                for group in groups:
                    logger.info(f"  • {group}")
        ...
        for i, (username, groups) in enumerate(all_admins.items(), 1):
            logger.info(f"[{i}/{len(all_admins)}] @{username} ({len(groups)} group{'s' if len(groups) > 1 else ''})")
