...
class ETLDataAnalyzer:
    ...
    def get_data_coverage_summary(self) -> Dict[str, Any]:
        ...
        if total > 0:
            summary['slack_coverage_pct'] = (summary['companies_with_slack'] / total) * 100
            summary['telegram_coverage_pct'] = (summary['companies_with_telegram'] / total) * 100
            summary['calendar_coverage_pct'] = (summary['companies_with_calendar'] / total) * 100
            summary['hubspot_coverage_pct'] = (summary['companies_with_hubspot'] / total) * 100
    ...
    def get_companies_with_multiple_sources(self, min_sources: int = 2) -> List[Dict[str, Any]]:
    ...
    def get_data_quality_metrics(self) -> Dict[str, Any]:
        ...
        if total_companies > 0:
            ...
            metrics['data_completeness_score'] = (total_sources / (total_companies * 4)) * 100  # 4 possible sources
        
        return metrics
    
    def export_company_list(self, output_file: str = "data/company_data_summary.csv") -> None:
        ...
        with open(output_file, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow([
                'Company Name', 'Base Company', 'Slack Channels', 'Telegram Chats', 
                'Calendar Meetings', 'HubSpot Deals', 'Total Sources', 'Data Quality'
            ])
            ...
            for company_name, company_data in self.data.get('companies', {}).items():
                ...
                total_sources = sum([slack_count, telegram_count, calendar_count, hubspot_count])
                
                data_quality = 'High' if total_sources >= 3 else 'Medium' if total_sources >= 2 else 'Low' if total_sources >= 1 else 'None'
                
                writer.writerow([
                    company_name, base_company, slack_count, telegram_count,
                    calendar_count, hubspot_count, total_sources, data_quality
                ])
    ...
    def print_summary_report(self) -> None:
        ...
        print(f"Slack Coverage: {coverage['companies_with_slack']} ({coverage.get('slack_coverage_pct', 0):.1f}%)")
        print(f"Telegram Coverage: {coverage['companies_with_telegram']} ({coverage.get('telegram_coverage_pct', 0):.1f}%)")
        print(f"Calendar Coverage: {coverage['companies_with_calendar']} ({coverage.get('calendar_coverage_pct', 0):.1f}%)")
        print(f"HubSpot Coverage: {coverage['companies_with_hubspot']} ({coverage.get('hubspot_coverage_pct', 0):.1f}%)")
        
        # Data quality metrics
        quality = self.get_data_quality_metrics()
        print(f"\n📈 DATA QUALITY METRICS")
        print(f"Companies with No Data: {quality['companies_with_no_data']}")
        print(f"Companies with Single Source: {quality['companies_with_single_source']}")
        print(f"Companies with Multiple Sources: {quality['companies_with_multiple_sources']}")
        print(f"Average Sources per Company: {quality['average_sources_per_company']:.2f}")
        ...
        if multi_source:
            ...
            for i, company in enumerate(multi_source[:10], 1):
                print(f"{i:2d}. {company['company_name']} ({company['source_count']} sources: {', '.join(company['sources'])})")
