...
def extract_telegram_groups():
    ...
    with open("data/telegram/DataExport_2025-08-19/lists/chats.html", 'r', encoding='utf-8') as f:
        content = f.read()
    ...
    for link in soup.find_all('a'):
        ...
        if 'chat_' in href:
            ...
            if len(parts) >= 3:
                ...
                if ('<>' in group_name or 
                    '&lt;&gt;' in group_name or  # HTML encoded <>
                    '/' in group_name or
                    ' - ' in group_name or  # Note: space-dash-space to avoid false positives
                    any(indicator in group_name.lower() for indicator in ['group', 'team', 'channel', 'community'])):
...
def assess_match_quality_strict(company, group_name):
    ...
    generic_only_groups = ["group", "community", "team", "channel", "chat", "general", "private"]
    if group_lower.strip() in generic_only_groups:
        return "none"
    
    # Skip individual names (DMs) - look for group dividers
    if not ('<>' in group_name or '&lt;&gt;' in group_name or '/' in group_name or ' - ' in group_name):
