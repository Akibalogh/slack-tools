...
class CalendarCommissionAnalysis:
    ...
    def get_company_meeting_data(self, company_name: str, days_back: int = 180) -> dict:
        ...
        company_meetings = self.calendar.search_meetings_for_company(
        ...
        # Also search for meetings with key people from the company
        key_people_meetings = self._search_for_key_people_meetings(
            company_name, days_back
        )

        # Combine all meetings
        all_meetings = company_meetings + key_people_meetings

        # Analyze overlap - find meetings where both company and team members were present
        overlapping_meetings = []
        for company_meeting in all_meetings:
            ...
            if team_participants:
                overlapping_meetings.append(
                    {"meeting": company_meeting, "team_participants": team_participants}
                )

        return {
            "company_name": company_name,
            "company_meetings": company_meetings,
            "key_people_meetings": key_people_meetings,
            "team_meetings": team_meetings,
            "overlapping_meetings": overlapping_meetings,
            "total_company_meeting_time": sum(
                m["duration_minutes"] for m in all_meetings
            ),
            "team_participation": self._calculate_team_participation(
                overlapping_meetings
            ),
        }

    def _search_for_key_people_meetings(
        self, company_name: str, days_back: int = 180
    ) -> list:
        """Search for meetings with key people from the company"""
        key_people_meetings = []

        # Define key people for specific companies
        company_key_people = {
            "black manta": ["finn", "oscar"],
            "vigil markets": ["soumya"],
            "launchnodes": ["launchnodes"],
            "bron": ["bron"],
            "komonode": ["igor", "gusarov"],
            "luganodes": ["luganodes"],
        }
        ...
        key_people = None
        for company_key, people in company_key_people.items():
            if company_key in company_lower or company_lower in company_key:
                key_people = people
                break

        if not key_people:
            return key_people_meetings

        # Search for meetings with each key person
        for person in key_people:
            try:
                # Search in Aki's calendar for meetings with this person
                events_result = (
                    self.calendar.service.events()
                    .list(
                        calendarId="aki@dlc.link",
                        timeMin=self._get_time_min(days_back),
                        timeMax=self._get_time_max(),
                        q=person,
                        singleEvents=True,
                        orderBy="startTime",
                    )
                ...
                for event in events:
                    # Process the event
                    meeting_data = self._process_calendar_event(event)
                    if meeting_data:
                        key_people_meetings.append(meeting_data)
    ...
    def _get_time_min(self, days_back: int) -> str:
        """Get ISO time string for days_back from now"""
        from datetime import datetime, timedelta

        time_min = datetime.now() - timedelta(days=days_back)
        return time_min.isoformat() + "Z"

    def _get_time_max(self) -> str:
        """Get ISO time string for now"""
        from datetime import datetime

        return datetime.now().isoformat() + "Z"

    def _process_calendar_event(self, event: dict) -> dict:
        """Process a Google Calendar event into our meeting format"""
        try:
            ...
            if isinstance(start_time, str) and isinstance(end_time, str):
                ...
                duration_minutes = 60  # Default for all-day events
            ...
            if "attendees" in event:
                for attendee in event["attendees"]:
                    ...
                    if email and email in self.team_emails.values():
    ...
    def analyze_launchnodes_deal(self) -> dict:
        ...
        print(
            f"  Total meeting time: {calendar_data['total_company_meeting_time']} minutes"
        )

        if calendar_data["overlapping_meetings"]:
            print(
                f"  Team participation meetings: {len(calendar_data['overlapping_meetings'])}"
            )
            print(
                f"  Team participation time: {calendar_data['team_participation']['total_time_minutes']} minutes"
            )

            print(f"\n👥 Team Participation (Calendar):")
            for name, percentage in calendar_data["team_participation"][
                "team_percentages"
            ].items():
    ...
    def _compare_calendar_vs_slack(self, calendar_data: dict, slack_data: dict):
        ...
        calendar_participants = set(
            calendar_data["team_participation"]["team_percentages"].keys()
        )
        slack_participants = set(slack_data["user_breakdown"].keys())

        print(
            f"  Calendar participants: {', '.join(calendar_participants) if calendar_participants else 'None'}"
        )
        print(
            f"  Slack participants: {', '.join(slack_participants) if slack_participants else 'None'}"
        )
        ...
        if total_calendar_time > 0 and total_slack_messages > 0:
            print(
                f"  📈 Calendar/Slack ratio: {total_calendar_time/total_slack_messages:.1f} min per message"
            )
    ...
    def get_commission_recommendation(self, company_name: str) -> dict:
        ...
        slack_analyzer = CommissionAnalyzer()
        slack_data = slack_analyzer.get_slack_activity(
    ...
    def _combine_calendar_and_slack(
        self, calendar_data: dict, slack_data: dict
    ) -> dict:
        ...
        if total_score > 0:
            normalized_scores = {
                name: (score / total_score) * 100 for name, score in team_scores.items()
            }
        else:
            normalized_scores = {}
    ...
    def _generate_recommendation(self, percentages: dict) -> str:
        ...
        if primary_percentage >= 80:
            ...
            return (
                f"Contested deal - {primary_name} leads with {primary_percentage:.1f}%"
            )
