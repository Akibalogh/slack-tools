...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('slack_ingest.log'),
...
class SlackIngest:
    def __init__(self, config_file: str = "config.json"):
        ...
        if not slack_token or slack_token == "xoxp-your-slack-token-here":
            raise ValueError("SLACK_TOKEN not found in .env file. Please add your Slack token to .env file")
    ...
    async def get_channel_history(self, channel_id: str, limit: int = 1000) -> List[Dict]:
    ...
    async def ingest_data(self, test_mode: bool = True, force_refresh: bool = False):
        ...
        for i, channel in enumerate(customer_channels, 1):
            ...
            print(f"{i:2d}. {name:<30} | Members: {member_count:2d} | Created: {created_date}")
        ...
        cursor.execute('SELECT COUNT(*) FROM conversations WHERE is_bitsafe = TRUE OR name = "gsf-app-dev"')
