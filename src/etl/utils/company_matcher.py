...
class CompanyMatcher:
    ...
    def __init__(self):
        ...
        self.company_suffixes = [
            ...
            'minter', 'mainnet', 'validator', 'canton', 'wallet', 'node',
            'bitsafe', 'safe', 'protocol', 'labs', 'tech', 'technologies',
            'systems', 'solutions', 'services', 'group', 'holdings'
        ]
        
        # Common prefixes to remove
        self.company_prefixes = [
            'the', 'a', 'an', 'new', 'advanced', 'global', 'international'
        ]
    ...
    def generate_name_variants(self, name: str) -> List[str]:
        ...
        filtered_variants = []
        generic_terms = {'gm', 'om', 'em', 'al', 'tr', 'ac', 'sc', 'rm', 'am', 'im', 'pt', 'ui', 'bc', 'fm', 'tm', 'a0', 'ig'}
    ...
    def match_company_to_channel(self, company_name: str, company_info: Dict, 
                                channel_name: str, channel_type: str = 'slack') -> bool:
        ...
        if channel_type == 'calendar':
            return self._match_calendar_meeting(company_name, company_info, channel_name)
    ...
    def find_best_matches(self, company_name: str, company_info: Dict, 
                         channels: Dict[str, Dict], channel_type: str = 'slack') -> List[Tuple[str, float]]:
        ...
        for channel_id, channel_data in channels.items():
            ...
            if self.match_company_to_channel(company_name, company_info, channel_name, channel_type):
                # Calculate confidence score
                confidence = self.calculate_confidence(company_name, company_info, channel_name, channel_type)
    ...
    def calculate_confidence(self, company_name: str, company_info: Dict, 
                           channel_name: str, channel_type: str = 'slack') -> float:
    ...
    def _match_telegram_chat(self, company_name: str, company_info: Dict, chat_name: str) -> bool:
    ...
    def _match_calendar_meeting(self, company_name: str, company_info: Dict, meeting_text: str) -> bool:
    ...
    def _match_hubspot_company(self, company_name: str, company_info: Dict, hubspot_company: str) -> bool:
        ...
        company_name_clean = company_name.replace('-', '').replace('_', '').replace(' ', '').lower()
        hubspot_clean = hubspot_company_lower.replace('inc', '').replace('llc', '').replace('corp', '').replace('ltd', '').replace('limited', '').replace('company', '').replace('co', '').replace(' ', '').replace('-', '').replace('_', '')
        ...
        business_suffixes = ['inc', 'llc', 'corp', 'ltd', 'limited', 'company', 'co', 'group', 'holdings', 'enterprises']
        business_prefixes = ['the', 'a', 'an', 'new', 'advanced', 'global', 'international']
        ...
        for suffix in business_suffixes:
            if hubspot_company_lower.endswith(suffix):
                ...
                if company_name_clean in hubspot_no_suffix or hubspot_no_suffix in company_name_clean:
                    return True
        
        # 7. Check against base company with business suffixes
        if company_info.get('base_company'):
            base_company_clean = company_info['base_company'].replace('-', '').replace('_', '').replace(' ', '').lower()
            for suffix in business_suffixes:
                if hubspot_company_lower.endswith(suffix):
                    hubspot_no_suffix = hubspot_company_lower[:-len(suffix)].strip()
                    if base_company_clean in hubspot_no_suffix or hubspot_no_suffix in base_company_clean:
                        return True
        
        return False
    
    def _match_email_domain_to_company(self, company_name: str, company_info: Dict, domain: str) -> bool:
        ...
        if company_info.get('calendar_domain') and domain_lower == company_info['calendar_domain'].lower():
            return True
        
        # 2. Company name in domain
        company_name_clean = company_name.lower().replace(' ', '').replace('-', '').replace('_', '')
        domain_clean = domain_lower.replace('.com', '').replace('.org', '').replace('.net', '').replace('.io', '')
