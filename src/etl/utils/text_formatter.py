...
class ETLTextFormatter:
    ...
    def _add_companies(self, companies: Dict[str, Any]):
        ...
        if isinstance(companies, dict):
            ...
            company_items = [(f"company_{i}", company) for i, company in enumerate(companies)]
        else:
            self.output_lines.append(f"ERROR: Unexpected companies data type: {type(companies)}")
            return
    ...
    def _add_company_section(self, company_name: str, company_data: Dict[str, Any]):
        ...
        if slack_channels:
            ...
            for channel in slack_channels:
                ...
                self.output_lines.append(f"  - {channel_name} ({message_count} messages)")
                
                # Show message summary for NotebookLM analysis (compact version)
                messages = channel_data.get('messages', [])
                if messages:
                    # Get unique senders and key message samples
                    unique_senders = set(msg.get('author', msg.get('user', 'Unknown')) for msg in messages)
                    ...
                    for i, msg in enumerate(recent_messages, 1):
                        ...
                        self.output_lines.append(f"      [{i}] [{timestamp}] {sender}: {text}")
        else:
        ...
        if telegram_chats:
            ...
            for chat in telegram_chats:
                ...
                self.output_lines.append(f"  - {chat_name} ({message_count} messages, {participants} participants)")
                ...
                if messages:
                    ...
                    for i, msg in enumerate(messages, 1):
                        ...
                        self.output_lines.append(f"      [{i:3d}] [{timestamp}] {sender}: {text}")
    ...
    def _add_company_analysis(self, company_name: str, company_data: Dict[str, Any]):
        ...
        total_telegram_messages = sum(ch.get('message_count', 0) for ch in telegram_chats)
        total_meetings = len(calendar_meetings)
        total_deals = len(hubspot_deals)
        
        # Engagement score calculation
        engagement_score = (total_slack_messages * 0.4 + 
        ...
        if engagement_score > 100:
            self.output_lines.append("  - HIGH PRIORITY: This company shows strong engagement")
        elif engagement_score > 50:
            self.output_lines.append("  - MEDIUM PRIORITY: Moderate engagement, consider follow-up")
        else:
            self.output_lines.append("  - LOW PRIORITY: Limited engagement, needs attention")
            
        if total_slack_messages > 200:
            self.output_lines.append("  - High Slack activity indicates strong relationship")
        if total_telegram_messages > 100:
            self.output_lines.append("  - High Telegram activity shows diverse communication")
        if total_deals > 0:
            self.output_lines.append("  - Active deals present - monitor closely")
        if total_meetings > 5:
            self.output_lines.append("  - Regular meetings indicate ongoing collaboration")
    ...
    def _add_notebooklm_summary(self, data: Dict[str, Any]):
        ...
        high_engagement = len([c for c in company_engagement if c['engagement_score'] > 100])
        medium_engagement = len([c for c in company_engagement if 50 <= c['engagement_score'] <= 100])
        low_engagement = len([c for c in company_engagement if c['engagement_score'] < 50])
        ...
        for i, company in enumerate(company_engagement[:5], 1):
            commission_potential = "HIGH" if company['engagement_score'] > 150 else "MEDIUM" if company['engagement_score'] > 75 else "LOW"
            self.output_lines.append(f"{i}. {company['name']} - {commission_potential} POTENTIAL")
    ...
    def format_company_summary(self, companies: Dict[str, Any]) -> str:
        ...
        for company_name, company_data in companies.items():
            ...
            lines.append(f"{company_name:<30} {slack:<8} {telegram:<10} {calendar:<10} {hubspot:<10}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Convert ETL JSON output to text format')
    parser.add_argument('input', help='Input JSON file')
    parser.add_argument('output', help='Output text file')
    parser.add_argument('--summary-only', action='store_true', help='Generate only summary table')
