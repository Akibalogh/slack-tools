...
class ETLSchemaValidator:
    ...
    def __init__(self, schema_path: str = None):
        ...
        if schema_path is None:
            schema_path = os.path.join(
                os.path.dirname(__file__), "..", "schemas", "etl_output_schema.json"
            )
    ...
    def _validate_business_rules(self, data: Dict[str, Any]) -> List[str]:
        ...
        if meta_companies != stats_companies:
            errors.append(
                f"Company count mismatch: metadata={meta_companies}, statistics={stats_companies}"
            )
        ...
        for company_name, company_data in companies.items():
            for source in ["slack", "telegram", "calendar", "hubspot"]:
                ...
                coverage_key = company_name.replace(
                    "-", "_"
                ...
                if has_data and not has_flag:
                    errors.append(
                        f"Company {company_name} has {source} data but coverage flag is False"
                    )
                elif not has_data and has_flag:
                    errors.append(
                        f"Company {company_name} has no {source} data but coverage flag is True"
                    )
    ...
    def _has_actual_data(self, source_data: Dict[str, Any], source: str) -> bool:
        ...
        if source == "slack":
            ...
            return len(channels) > 0 and any(
                len(channel.get("messages", [])) > 0 for channel in channels
            )
        elif source == "telegram":
            chats = source_data.get("chats", [])
            return len(chats) > 0 and any(
                len(chat.get("messages", [])) > 0 for chat in chats
            )
    ...
    def get_validation_summary(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "data_summary": {
                ...
                "total_errors": metadata.get("performance_stats", {}).get(
                    "total_errors", 0
                ),
            },
...
def main():
    ...
    parser.add_argument(
        "--summary", action="store_true", help="Show validation summary"
    )
