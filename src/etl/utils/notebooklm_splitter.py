...
class NotebookLMSplitter:
    ...
    def _create_commission_opportunities(self, data: Dict[str, Any]) -> str:
        ...
        high_tier = len([c for c in company_engagement if c['commission_tier'] == 'HIGH'])
        medium_tier = len([c for c in company_engagement if c['commission_tier'] == 'MEDIUM'])
        low_tier = len([c for c in company_engagement if c['commission_tier'] == 'LOW'])
        
        total_estimated_commission = sum(c['estimated_commission'] for c in company_engagement)
