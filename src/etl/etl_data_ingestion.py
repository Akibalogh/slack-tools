...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("logs/etl_ingestion.log"), logging.StreamHandler()],
...
def process_telegram_chat_worker(
    chat_dir: str, chats_dir: str
...
class DataETL:
    def __init__(
        ...
        self.output_file = (
            "output/notebooklm/etl_output.txt"  # Main file for NotebookLM
        )
    ...
    def _match_calendar_by_email_domain(
        self, company_name: str, company_info: Dict, attendee_domains: set
    ) -> bool:
        ...
        for domain in attendee_domains:
            ...
            if (
                ...
                and domain == company_info["calendar_domain"].lower()
            ):
                return True
            ...
            if company_info.get("base_company"):
                ...
                if (
                    base_company_clean in domain_clean
                    or domain_clean in base_company_clean
                ):
                    return True
            ...
            if company_info.get("variant_type"):
                ...
                if variants_str:
                    ...
                    for variant in variants:
                        ...
                        if (
                            variant_clean in domain_clean
                            or domain_clean in variant_clean
                        ):
                            return True
    ...
    def _match_calendar_by_attendee_domains(
        self, company_name: str, company_info: Dict, attendee_domains: set
    ) -> bool:
    ...
    def load_company_mapping(self) -> Dict[str, Any]:
        ...
        with open(self.company_mapping_file, "r", encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                if filtered_companies and not self._is_company_in_filtered_list(
                    company_name, filtered_companies
                ):
        ...
        logger.info(
            f"Loaded {len(companies)} companies from mapping file (filtered from provided list)"
        )
        return companies
    ...
    def _load_filtered_company_list(self) -> List[str]:
        ...
        try:
            ...
            logger.info(
                f"Loaded {len(companies)} companies from filter list (validator node format supported)"
            )
            return companies
        except Exception as e:
    ...
    def _is_company_in_filtered_list(
        self, company_name: str, filtered_companies: List[str]
    ) -> bool:
    ...
    def _process_single_chat(
        self, chat_dir: str, chats_dir: str
    ) -> Optional[Dict[str, Any]]:
        ...
        try:
                            ...
                            chat_messages.append(
                                {
                                    ...
                                    "message_type": (
                                        "service" if is_service else "regular"
                                    ),
            ...
            if chat_participants and not any(
                indicator in original_chat_name.lower()
                for indicator in ["company", "team", "group", "channel"]
            ):
            ...
            if chat_messages:
                ...
                unique_authors = list(
                    set(m["author"] for m in chat_messages if m["author"] != "System")
                )
                ...
                return {
                    ...
                    "last_message_time": max(
                        [m["timestamp"] for m in chat_messages if m["timestamp"]],
                        default="",
                    ),
    ...
    def ingest_telegram_data(self) -> Dict[str, Any]:
        ...
        if self.quick_mode:
            ...
            logger.info(
                f"QUICK MODE: Processing only first {len(chat_dirs)} of {original_count} chat directories"
            )
        else:
        ...
        for i in range(0, len(chat_dirs), self.batch_size):
            ...
            if self.use_multiprocessing:
                with ProcessPoolExecutor(max_workers=self.max_workers) as executor:
                    ...
                    future_to_chat = {
                        executor.submit(
                            process_telegram_chat_worker, chat_dir, chats_dir
                        ): chat_dir
                        ...
                        try:
                            ...
                            if result:
                                ...
                                if processed_count % 50 == 0:
                                    print(
                                        f"\r🔄 Processed {processed_count} chats so far...",
                                        end="",
                                        flush=True,
                                    )
                        except Exception as e:
                ...
                with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                    ...
                    future_to_chat = {
                        executor.submit(
                            self._process_single_chat, chat_dir, chats_dir
                        ): chat_dir
                        for chat_dir in batch
                    }
            ...
            # Log progress
            print(
                f"✅ Completed batch {batch_num}. Processed: {processed_count}, Errors: {error_count}"
            )
    ...
    def ingest_calendar_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if calendar.authenticate():
                ...
                for meeting in meetings:
                    ...
                    if attendee_domains:
                        for company_name, company_info in self.companies.items():
                            if self._match_calendar_by_attendee_domains(
                                company_name, company_info, attendee_domains
                            ):
                    ...
                    if not matched_company:
                        ...
                        for company_name, company_info in self.companies.items():
                            if self.matcher.match_company_to_channel(
                                company_name, company_info, meeting_text, "calendar"
                            ):
                                matched_company = company_name
                                break
                ...
                logger.info(
                    f"Ingested calendar data for {len(calendar_data)} companies"
                )
            else:
            ...
            if hubspot.load_export_files():
                ...
                for company_name, deals in company_deals.items():
                    hubspot_data[company_name] = {
                        ...
                        "total_value": sum(
                            float(deal.get("deal_value", 0) or 0) for deal in deals
                        ),
                        "active_deals": len(
                            [
                                d
                                for d in deals
                                if d.get("deal_stage", "").lower()
                                not in ["closed won", "closed lost", "closed"]
                            ]
                        ),
                    }
    ...
    def match_data_to_companies(self) -> Dict[str, Any]:
        ...
        for company_name, company_info in self.companies.items():
            ...
            if company_name in self.calendar_data:
                matched_data[company_name]["calendar_meetings"].extend(
                    self.calendar_data[company_name]
                )
            ...
            for deal_company, deal_info in self.hubspot_data.items():
                ...
                if self.matcher.match_company_to_channel(
                    company_name, company_info, deal_company, "hubspot"
                ):
    ...
    def generate_summary_stats(self, matched_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "total_calendar_meetings": sum(
                len(meetings) for meetings in self.calendar_data.values()
            ),
...
"""
            ...
            logger.info(
                f"Telegram: {stats.get('companies_with_telegram', 0)} companies"
            )
            logger.info(
                f"Calendar: {stats.get('companies_with_calendar', 0)} companies"
            )
    ...
    def _generate_fallback_stats(self) -> Dict[str, Any]:
        ...
        return {
            "total_companies": len(self.companies),
            "companies_with_slack": len(
                [c for c in self.companies.values() if c.get("slack")]
            ),
            "companies_with_telegram": len(
                [c for c in self.companies.values() if c.get("telegram")]
            ),
            "companies_with_calendar": len(
                [c for c in self.companies.values() if c.get("calendar")]
            ),
            "companies_with_hubspot": len(
                [c for c in self.companies.values() if c.get("hubspot")]
            ),
            "total_slack_channels": len(self.slack_data),
            "total_telegram_chats": len(self.telegram_data),
            "total_calendar_meetings": sum(
                len(c.get("meetings", [])) for c in self.calendar_data.values()
            ),
            "total_hubspot_deals": len(self.hubspot_data),
        }
