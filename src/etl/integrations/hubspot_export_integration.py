...
class HubSpotExportIntegration:
    ...
    def load_export_files(self) -> bool:
        ...
        if not os.path.exists(self.export_directory):
            logger.warning(f"HubSpot export directory not found: {self.export_directory}")
            return False
        ...
        try:
            ...
            if not files:
                logger.warning(f"No HubSpot export files found in {self.export_directory}")
                return False
    ...
    def _load_file(self, file_path: str):
        ...
        try:
            ...
            if 'deal' in filename:
                ...
                logger.info(f"Loaded {len(self.contacts_data)} contacts from {file_path}")
            elif 'compan' in filename:
                self.companies_data.extend(self._process_companies(df))
                logger.info(f"Loaded {len(self.companies_data)} companies from {file_path}")
            else:
                # Try to auto-detect based on columns
                if self._is_deals_data(df):
                    self.deals_data.extend(self._process_deals(df))
                    logger.info(f"Auto-detected and loaded {len(self.deals_data)} deals from {file_path}")
                elif self._is_contacts_data(df):
                    self.contacts_data.extend(self._process_contacts(df))
                    logger.info(f"Auto-detected and loaded {len(self.contacts_data)} contacts from {file_path}")
                elif self._is_companies_data(df):
                    self.companies_data.extend(self._process_companies(df))
                    logger.info(f"Auto-detected and loaded {len(self.companies_data)} companies from {file_path}")
                else:
    ...
    def _is_contacts_data(self, df: pd.DataFrame) -> bool:
        ...
        return any(col.lower() in ' '.join(df.columns).lower() for col in contact_columns)
    ...
    def _is_companies_data(self, df: pd.DataFrame) -> bool:
        ...
        return any(col.lower() in ' '.join(df.columns).lower() for col in company_columns)
    ...
    def _process_deals(self, df: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        for _, row in df.iterrows():
            deal = {
                'deal_name': self._get_value(row, ['Deal Name', 'deal name', 'deal_name', 'name', 'title']),
                'company_name': self._get_value(row, ['Company Name', 'company', 'company_name', 'account', 'account_name']),
                'deal_stage': self._get_value(row, ['Deal Stage', 'stage', 'deal_stage', 'pipeline_stage', 'status']),
                'deal_value': self._get_value(row, ['Amount', 'amount', 'deal_value', 'value', 'revenue']),
                'close_date': self._get_value(row, ['Close Date', 'close date', 'close_date', 'expected_close', 'close']),
                'deal_owner': self._get_value(row, ['Deal owner', 'owner', 'deal_owner', 'assigned_to', 'user', 'deal owner']),
                'description': self._get_value(row, ['Use case', 'description', 'notes', 'comments', 'use case']),
                'source': self._get_value(row, ['Sourced by', 'source', 'lead_source', 'origin', 'sourced by']),
                'created_date': self._get_value(row, ['created', 'created_date', 'date_created']),
                'last_modified': self._get_value(row, ['modified', 'last_modified', 'updated']),
                'priority': self._get_value(row, ['Priority', 'priority']),
                'estimated_fees': self._get_value(row, ['Estimated Fees ($)', 'estimated fees ($)', 'estimated_fees']),
                'btc_fund_size': self._get_value(row, ['BTC Fund Size ($Mn)', 'btc fund size ($mn)', 'btc_fund_size']),
            }
            ...
            if deal_stage:
                ...
                if not (deal_stage_lower in ['commit', 'closed won', 'closed-won', 'closedwon'] or 
                        'commit' in deal_stage_lower or 'closed won' in deal_stage_lower):
                    continue
    ...
    def _process_contacts(self, df: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        for _, row in df.iterrows():
            contact = {
                'first_name': self._get_value(row, ['first name', 'first_name', 'first']),
                ...
                'job_title': self._get_value(row, ['title', 'job_title', 'position', 'role']),
    ...
    def _process_companies(self, df: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        for _, row in df.iterrows():
            company = {
                'company_name': self._get_value(row, ['company name', 'company_name', 'name', 'account']),
                'domain': self._get_value(row, ['domain', 'website', 'url']),
                'industry': self._get_value(row, ['industry', 'sector', 'vertical']),
                'city': self._get_value(row, ['city', 'location']),
                'state': self._get_value(row, ['state', 'province', 'region']),
                'country': self._get_value(row, ['country', 'nation']),
                'employee_count': self._get_value(row, ['employees', 'employee_count', 'size']),
                'annual_revenue': self._get_value(row, ['revenue', 'annual_revenue', 'sales']),
                'created_date': self._get_value(row, ['created', 'created_date', 'date_created']),
                'last_modified': self._get_value(row, ['modified', 'last_modified', 'updated']),
            }
