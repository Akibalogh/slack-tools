...
class TelegramMatchVerifier:
    ...
    def find_matches(self, companies: List[str], groups: Dict[str, str]) -> List[Tuple[str, str, str, str]]:
        """
        ...
        for company in companies:
            ...
            for chat_dir, group_name in groups.items():
                ...
                if quality != "none" and (best_match is None or self._is_better_match(quality, best_quality)):
    ...
    def generate_verification_table(self, matches: List[Tuple[str, str, str, str]]) -> str:
        """Generate a verification table"""
        table = []
        table.append("# Telegram Match Verification Table")
        table.append("")
        table.append("| Company | Chat Dir | Telegram Group Name | Match Quality | Notes |")
        table.append("|---------|----------|-------------------|---------------|-------|")
        
        for company, chat_dir, group_name, quality in matches:
            if quality != "none":
                table.append(f"| {company} | `{chat_dir}` | {group_name} | {quality} | |")
            else:
                table.append(f"| {company} | | | | No match found |")
                
        return "\n".join(table)
    
    def save_verification_table(self, table: str, output_file: str = "telegram_match_verification.md"):
