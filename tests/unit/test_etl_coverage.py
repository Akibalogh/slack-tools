...
with patch("logging.FileHandler"), patch("logging.basicConfig"), patch("os.makedirs"):
...
class TestETLCoverage:
    ...
    def setup_method(self):
        ...
        with patch("logging.FileHandler"), patch("logging.basicConfig"), patch(
            "os.makedirs"
        ):
            self.etl = DataETL()
    ...
    def test_load_company_mapping_success(self):
        ...
        with patch("builtins.open", mock_open()), patch(
            "csv.DictReader", return_value=mock_data
    ...
    def test_etl_error_handling(self):
        ...
        with patch.object(
            self.etl, "load_company_mapping", side_effect=Exception("Test error")
        ):
            try:
