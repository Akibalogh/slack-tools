...
class TestETLFunctionality:
    ...
    def mock_etl(self, temp_dir):
        ...
        with patch('src.etl.etl_data_ingestion.sqlite3.connect'), \
             patch('src.etl.etl_data_ingestion.os.path.exists', return_value=True):
    ...
    def test_etl_workflow_mock(self, mock_etl, temp_dir):
        ...
        with patch.object(mock_etl, 'load_company_mapping') as mock_load, \
             patch.object(mock_etl, 'ingest_slack_data') as mock_slack, \
             patch.object(mock_etl, 'ingest_telegram_data') as mock_telegram, \
             patch.object(mock_etl, 'ingest_calendar_data') as mock_calendar, \
             patch.object(mock_etl, 'ingest_hubspot_data') as mock_hubspot, \
             patch.object(mock_etl, 'match_data_to_companies') as mock_match, \
             patch.object(mock_etl, 'generate_summary_stats') as mock_stats:
            ...
            mock_match.return_value = {'test-company': {'slack_channels': [], 'telegram_chats': []}}
