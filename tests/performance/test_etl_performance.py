...
class TestETLPerformance:
    ...
    def large_company_mapping(self, temp_workspace):
        ...
        for i in range(1000):
            ...
            companies.append(f"{company_name},{company_name}::1220409a9fcc5ff6422e29ab978c22c004dde33202546b4bcbde24b25b85353366c2,{company_name}-channel,{company_name},{company_name}.com,base,{company_name}")
    ...
    def large_slack_database(self, temp_workspace):
        ...
        # 100 conversations
        for i in range(100):
            cursor.execute(f"INSERT INTO conversations (id, name, type) VALUES ({i+1}, 'test-company-{i:04d}-channel', 'channel')")
        
        # 10 users
        for i in range(10):
            cursor.execute(f"INSERT INTO users (id, real_name, display_name) VALUES ('U{i:03d}', 'User{i:03d}', 'User{i:03d}')")
        
        # 1000 messages (10 per conversation)
        message_id = 1
        for conv_id in range(1, 101):
            for msg_num in range(10):
                cursor.execute(f"INSERT INTO messages (id, conversation_id, author, text, timestamp) VALUES ({message_id}, {conv_id}, 'U{msg_num % 10:03d}', 'Test message {msg_num} in conversation {conv_id}', {1640995200 + message_id})")
                message_id += 1
    ...
    def test_etl_quick_mode_performance(self, temp_workspace, large_company_mapping, large_slack_database, large_telegram_data):
        ...
        etl.output_file = os.path.join(temp_workspace, 'output', 'notebooklm', 'etl_output_quick.txt')
        etl.archive_dir = os.path.join(temp_workspace, 'output', 'notebooklm', 'archive')
        ...
        assert memory_used < 500, f"Memory usage was {memory_used:.2f}MB, should be < 500MB"
        ...
        assert output_size < 100 * 1024 * 1024, f"Output file too large: {output_size / 1024 / 1024:.2f}MB"
    
    def test_etl_full_mode_performance(self, temp_workspace, large_company_mapping, large_slack_database, large_telegram_data):
        ...
        etl.slack_db_path = large_slack_database
        etl.telegram_chats_dir = large_telegram_data
        etl.output_file = os.path.join(temp_workspace, 'output', 'notebooklm', 'etl_output_full.txt')
        etl.archive_dir = os.path.join(temp_workspace, 'output', 'notebooklm', 'archive')
        etl.logs_dir = os.path.join(temp_workspace, 'logs')
        ...
        assert memory_used < 1000, f"Memory usage was {memory_used:.2f}MB, should be < 1000MB"
    ...
    def test_etl_memory_usage(self, temp_workspace, large_company_mapping, large_slack_database, large_telegram_data):
        ...
        etl.slack_db_path = large_slack_database
        etl.telegram_chats_dir = large_telegram_data
        etl.output_file = os.path.join(temp_workspace, 'output', 'notebooklm', 'etl_output_memory.txt')
        etl.archive_dir = os.path.join(temp_workspace, 'output', 'notebooklm', 'archive')
        etl.logs_dir = os.path.join(temp_workspace, 'logs')
        ...
        assert max_memory < 500, f"Peak memory usage was {max_memory:.2f}MB, should be < 500MB"
        assert avg_memory < 300, f"Average memory usage was {avg_memory:.2f}MB, should be < 300MB"
    
    def test_etl_concurrent_workers(self, temp_workspace, large_company_mapping, large_telegram_data):
        ...
        for workers in worker_configs:
            ...
            etl.company_mapping_file = large_company_mapping
            etl.telegram_chats_dir = large_telegram_data
            etl.output_file = os.path.join(temp_workspace, f'output_workers_{workers}.txt')
        ...
        assert results[2] <= results[1] * 1.5, "2 workers should be reasonably faster than 1 worker"
        assert results[4] <= results[2] * 1.2, "4 workers should be faster than 2 workers"
    
    def test_etl_batch_size_impact(self, temp_workspace, large_company_mapping, large_telegram_data):
        ...
        for batch_size in batch_sizes:
            ...
            etl.company_mapping_file = large_company_mapping
            etl.telegram_chats_dir = large_telegram_data
            etl.output_file = os.path.join(temp_workspace, f'output_batch_{batch_size}.txt')
            etl.archive_dir = os.path.join(temp_workspace, 'archive')
            etl.logs_dir = os.path.join(temp_workspace, 'logs')
        ...
        for batch_size, duration in results.items():
            assert duration > 0, f"Batch size {batch_size} should complete in reasonable time"
            assert duration < 60, f"Batch size {batch_size} took {duration:.2f}s, should be < 60s"
