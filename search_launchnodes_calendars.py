...
def main():
    ...
    for calendar_id in personal_calendars:
        try:
            ...
            meetings = calendar.search_meetings_for_company('Launchnodes', days_back=180)
            if meetings:
            ...
            variations = ['LaunchNodes', 'launchnodes', 'Launch Node', 'Jaydeep', 'BitSafe']
    ...
    if all_meetings:
        print(f"  Total meeting time: {sum(m['duration_minutes'] for m in all_meetings)} minutes")
        ...
        print(f"\n👥 Attendee Summary:")
        for attendee, minutes in sorted(attendees.items(), key=lambda x: x[1], reverse=True):
